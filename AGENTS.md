...
`src/htmls/demo.html` is the served entry that showcases multiple `<web-ide>` instances. Runtime code and assets live in `src/s/`: `cssNjs` contains `pyide.js`, `pyideCore.js`, and formatting helpers; `ace` and `aceMods` ship the editor builds and extensions; `pyodideMods` plus `skulpt` cover execution workers; `katex` adds math rendering. Place new static files beside similar assets so the static copy plugin picks them up. Builds land in `dist/`, while Playwright specs and helpers are under `tests/`; keep `test-results/` untracked.
...
`pyide.js` registers `<web-ide>` and lazy-loads `pyideCore.js` when a panel opens. `pyideCore` wires Ace to the Python execution workers, coordinates run/format events, and updates the UI. Execution flows through `pyodide-worker-manager.js` into `pyodide-worker.js`; formatting delegates to `ruffFormatterWorker.js`. Keep additions async-friendly so the UI thread stays responsive.
