...
let MD5 = {
    ...
    ff: function (a, b, c, d, k, s, t) {
        let q = ((b & c) | ((~b) & d));
        let z = add32(add32(a, q), add32(k, t));
    ...
    mix: function (blocks, vector) {
        ...
        let a = vector[0], b = vector[1], c = vector[2], d = vector[3];
        let ff = MD5.ff, gg = MD5.gg, hh = MD5.hh, ii = MD5.ii;
    ...
    load: function (buffer, input, start, howmany, support_unicode) {
        ...
        let values = input.substr(start, howmany), i = values.length;
        let chars = buffer, char_val, q = 0;
        let temp, lower;
    ...
    hash: function (input) {
        ...
        let vector = [0x67452301, 0xEFCDAB89, 0x98BADCFE, 0x10325476];

        let readsize = 512; // How many bytes to read each time from input
        let position = 0;

        // The buffer (8-bit chunks read from input)
        let buffer = [];
        let chunk, i, blocks, last = 0, total = 0;
...
export const md5 = (text) => MD5.hex(MD5.hash(text));
